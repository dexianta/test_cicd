...
```
dbx execute --cluster-name=<name of interactive cluster> --job=fake_project-sample-integration-test
```

For a test on a automated job cluster, use `launch` instead of `execute`:
```
dbx execute --job=fake_project-sample-integration-test
```
...
```
dbx launch --job=fake_project-sample
```
